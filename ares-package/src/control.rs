...
impl<W> AppendControl for ArBuilder<W> where W: IoWrite {
    fn append_control(&mut self, info: &ControlInfo) -> std::io::Result<()> {
        ...
        drop(builder);
        return self.append(&ArHeader::new(b"control.tar.gz".to_vec(), control_tar_gz.len() as u64),
